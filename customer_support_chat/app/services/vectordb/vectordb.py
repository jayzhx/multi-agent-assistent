...
class VectorDB:
    ...
    def create_embeddings(self, docs):
        for doc_id, content, url in docs:
            if content is None:
                logger.warning(f"跳过 doc_id {doc_id}，因为内容为空。")
                continue

            chunks = recursive_character_splitting(content)
            for i, chunk in enumerate(chunks):
                try:
                    logger.info(f"正在为 doc_id: {doc_id} 的第 {i+1} 个分块生成嵌入向量")
                    embedding = self.generate_embedding(chunk)
                    self.upsert_vector(doc_id, chunk, embedding, url, i)
                except Exception as e:
                    logger.error(f"为 doc_id: {doc_id} 的第 {i+1} 个分块生成或存储嵌入向量失败，错误：{str(e)}")

        logger.info("所有文档的嵌入向量生成已完成")
